...
use druid::{BoxConstraints, Data, Env, Event, EventCtx, LayoutCtx, LifeCycle, LifeCycleCtx, PaintBrush, PaintCtx, RenderContext, UpdateCtx, Widget, WidgetPod, KeyOrValue, Color};
...
impl<T: Data> Widget<T> for Container<T> {
    ...
    fn paint(&mut self, paint_ctx: &mut PaintCtx, data: &T, env: &Env) {
        ...
        if let Some(background) = &self.background {
            paint_ctx.fill(panel, &PaintBrush::Color(background.resolve(env)));
        };
